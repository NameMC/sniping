...
So, how exactly do you "delete transfer" a name from a Game Pass account? Well, despite the deceptive name, you don't actually delete the account as you would with a gift card account. This method usually costs $200 to perform, but since it's nearing the end of its lifespan, I'll share it with you (the reader) and anyone else who uses this guide.

To delete transfer a name using a Game Pass account, you'll first have to acquire an "unmigrated account". These are accounts that have not yet been migrated to Microsoft. Well, simply put, they're Mojang accounts. If you migrated a Mojang account to a Game Pass account that has an expired subscription, the name will be freed up, allowing you to transfer the name to another account. It is highly recommended to consult a "swapper" who can safely transfer your name using this method. 
